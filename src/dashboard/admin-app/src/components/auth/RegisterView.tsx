...
export default function RegisterView({ onRegister, onBack }: RegisterViewProps) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      await onRegister({ email, password, full_name: name || undefined, organization: org || undefined })
    } catch (err: any) {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2.5 mb-2">
        ...
        <h2 className="text-[22px] font-semibold">Create Account</h2>
      </div>
      <p className="text-sm mb-7" style={{ color: '#a49fc4' }}>Join the Pragma platform</p>
      {error && (
      ...
      <form onSubmit={handleSubmit}>
        <div className="mb-[18px]">
          ...
          <input type="email" value={email} onChange={(e) => setEmail(e.target.value)}
            ...
            style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', color: '#f0eef8' }}
            onFocus={(e) => e.target.style.borderColor = 'rgba(226,184,77,0.3)'}
        ...
        <div className="mb-[18px]">
          <label className="block text-[13px] mb-1.5" style={{ color: '#a49fc4' }}>Full Name</label>
        ...
        <div className="mb-[18px]">
          <label className="block text-[13px] mb-1.5" style={{ color: '#a49fc4' }}>Organization</label>
        ...
        <button type="submit" disabled={busy}
          ...
          style={{ background: 'linear-gradient(135deg, #e2b84d, #c9a33e)', color: '#0b0b12' }}
        >
          {busy ? <i className="fas fa-circle-notch fa-spin" /> : <i className="fas fa-user-plus" />}
          Create Account
        </button>
      </form>
