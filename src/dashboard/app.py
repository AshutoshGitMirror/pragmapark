...
app.layout = html.Div(
    [
        html.H1(
            "Gemini Smart Parking — 6-Layer Hybrid System",
            style={
                "textAlign": "center",
                "color": "#2c3e50",
                "marginBottom": 30,
        ...
        html.Div(
            [
                html.H3(
                    "Layer 7 — Micro Slot Grid (per-slot occupancy heatmap)",
        ...
        dcc.Interval(
            id="lot-dropdown-interval", interval=30000, n_intervals=0
        ),
    ],
    style={
...
def _get_features():
    ...
    try:
        ...
        logger.exception("event=dashboard.data.load.failed")
...
def update_blockchain(_):
    ...
    fig.add_annotation(
        text=f"Pending TX: {pending} | Total Blocks: {chain_len}",
        xref="paper",
        yref="paper",
        x=0.5,
        y=1.05,
...
def update_rl(_):
    ...
    fig.add_trace(
        go.Scatter(
            x=steps, y=occs, name="Occupancy", line=dict(color="#3498db")
        ),
        secondary_y=False,
    )
    fig.add_trace(
        go.Scatter(
            x=steps, y=prices, name="Dynamic Price", line=dict(color="#e74c3c")
        ),
        secondary_y=True,
    )
    fig.add_annotation(
        text=f"RL Agent: {'active' if agent_avail else 'heuristic fallback'}",
        xref="paper",
        yref="paper",
        x=0.5,
        y=1.05,
...
def update_marl(_):
    ...
    fig.update_layout(
        ...
        yaxis2=dict(
            title="Price ($)", overlaying="y", side="right", range=[0, 60]
        ),
...
_STATE_MAP = {
    SlotState.OCCUPIED: 3,
    SlotState.RESERVED: 2,
    SlotState.MAINTENANCE: 0.5,
    SlotState.AVAILABLE: 1,
...
def update_slot_grid(lot_id, _):
    ...
    fig = go.Figure(
        data=go.Heatmap(
            ...
            customdata=[
                [
                    _LABEL_MAP.get(v, "Empty") if v is not None else "Empty"
                    for v in row
                ]
                for row in heat
            ],
    ...
    fig.update_layout(
        title=f"{lot_id} — {occ['occupied_slots']}/{
            occ['total_slots']
        } occupied ({occ['occupancy_rate'] * 100:.1f}%)",
        template="plotly_white",
