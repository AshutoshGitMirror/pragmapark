...
class RealisticParkingSensorSimulator:
    """Realistic time-series synthetic IoT sensor data generator for smart parking.

    Models:
    - Temporal patterns (business hour commutes, day/night cycles, weekends).
    - Spatial patterns (entrance proximity filling sequence).
    - Physical sensor signal model for ultrasonic distance readings (noise + dropouts + drift).
    - Physical camera model (ambient light dependency, weather degradation, occlusions).
    - Environmental interference (stochastic weather factor and rain/storm noise bursts).
    - Pure NumPy implementation.
    """

    def __init__(self, zone_id: str, capacity: int, base_price: float = 10.0,
                 entrance_skew: float = 15.0, us_noise_std: float = 0.05,
                 us_dropout_prob: float = 0.01, us_drift_rate: float = 0.0001,
                 vis_occlusion_prob: float = 0.02, vis_drift_rate: float = -0.0001):
    ...
    def get_base_occupancy_rate(self, dt: datetime) -> float:
        ...
        if not is_weekend:
            ...
            occ_rate = 0.12 + 0.68 * (0.45 * morning_peak + 0.55 * evening_peak)
        else:
    ...
    def get_weather_factor(self, dt: datetime) -> float:
        """Generate a time-varying weather factor representing rain/snow/storm interference.
        
        Outputs weather value in [0, 1]. A higher value causes heavier sensor degradation.
        Includes diurnal variance and transient storm bursts.
        """
        # Base weather factor using seasonal sinusoid (higher in winter/autumn months)
    ...
    def get_ground_truth_occupancy(self, base_rate: float) -> np.ndarray:
        """Simulate spatial patterns where spots closer to the entrance (low index) fill first.
        
        P(occupied_slot_i) = sigmoid(entrance_skew * (base_rate - i / capacity))
        """
    ...
    def sample_step(self, dt: datetime) -> List[DualSensorReading]:
        ...
        # Ultrasonic drifts positive (raising distance read), Vision drifts negative (lowering confidence)
        self.us_bias += np.random.normal(self.us_drift_rate, 0.0001, self.capacity)
        self.vis_bias += np.random.normal(self.vis_drift_rate, 0.0001, self.capacity)
        ...
        for i in range(self.capacity):
            ...
            if is_us_dropout:
                # Dropout means we fail to get a reflection, returning empty floor distance
                dist = self.D_floor
            else:
                base_dist = self.D_car if gt else self.D_floor
                dist = base_dist + np.random.normal(0, us_noise_eff) + self.us_bias[i]
            ...
            if is_vis_occluded:
                ...
                # Camera accuracy degrades based on dark conditions and bad weather
