...
class ParkingEventExtractor:
    ...
    def extract_events(
        self, df: pd.DataFrame, occupancy_col: str = "occupancy_rate"
    ) -> pd.DataFrame:
        result = df.copy()
        ...
        result["pe_departure_rate"] = g[occupancy_col].transform(
            lambda s: (
                (-s.diff()).clip(lower=0).rolling(4, min_periods=1).mean()
            )
        )
        result["pe_net_flux"] = g[occupancy_col].transform(
        ...
        result["pe_anomaly"] = (
            (result[occupancy_col] - mean_occ).abs() > 2 * std_occ
        ).astype(float)
        result["pe_anomaly"] = result["pe_anomaly"].fillna(0)
        result["pe_change_point"] = self._detect_change_points(
            df, occupancy_col
        )
        return result

    def _detect_change_points(
        self, df: pd.DataFrame, occ_col: str
    ) -> pd.Series:
