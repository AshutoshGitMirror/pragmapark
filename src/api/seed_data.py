...
Exercises all 6 architecture layers:
  ...
  RL        — demand-responsive pricing (price inversely correlated
              with occupancy)
...
# Tuple: (lot_id, name, address, city, slots, lat, lng, base_price,
#         price_cap, occ_pct)
# ---------------------------------------------------------------------------
SEED_LOTS = [
...
_SEED_LOT_MAP = {
    r[0]: {"base_price": r[7], "price_cap": r[8], "slots": r[4], "city": r[3]}
    for r in SEED_LOTS
}

# Each lot's behavioural profile:
# (peak_amplitude, morning_peak_hour, evening_peak_hour,
# base_trough, weekend_factor)
# Peak amplitude: how much above trough at peak (as fraction of occ_pct)
# Some lots are commuter-heavy (twin-peak), others are leisure (single
# afternoon peak)
LOT_PROFILE = {
    "A1": {
        "type": "mixed_retail",
...
def _weather_for(d: date_type) -> dict:
    """Deterministic weather: temperature, rain prob, intensity."""
    ...
    rain_prob = (
        0.15
        + 0.25 * (1 + math.sin(2 * math.pi * (day_of_year - 60) / 365)) / 2
    )
...
def _occupancy_diurnal(hour: float, profile: dict) -> float:
    ...
    Returns a value between 0 and 1 that modulates the base occupancy.
    """
    ...
    if shape == "twin":
        ...
        return trough + (1 - trough) * max(
            0, math.sin(math.pi * (hour - 5) / 16)
        )
...
def _make_session_id(lot_id: str, idx: int, day_offset: int) -> str:
    ...
    h = hashlib.sha256(raw.encode()).hexdigest()[:14].upper()
    return f"S{h}"


def _make_tx_hash() -> str:
    ts = datetime.now(timezone.utc).timestamp()
    raw = f"tx-{_seed_rng.getrandbits(64)}-{ts}"
...
def seed_all(session, days: int = 30) -> dict:
    ...
    # ─── Step 2: Create lots ───
    lot_map = {}  # lot_id -> {occ_pct, slots, profile, base_price,
    #              price_cap, type_keys...}
    ...
    for lot_id, info in lot_map.items():
        ...
        for zi in range(nzones):
            z = MicroZone(
                ...
                name=znames[zi],
                description=(
                    f"{znames[zi]} — capacity ~{slots_count // nzones}"
                ),
        ...
        for si in range(slots_count):
            ...
            # Slot type distribution: 70% regular, 10% premium, 5% EV, 5%
            # handicap, 10% covered
            ...
            session.flush()
            slot_list.append(
                {"slot_id": ms.id, "slot_index": si, "zone_id": zones[zi].id}
            )
    ...
    for day_offset in range(days, -1, -1):
            ...
            for lot_id, info in lot_map.items():
                ...
                if minute_offset % 60 == 0 and _seed_rng.random() < 0.15:
                    ...
                    if slot_pool_lot:
                        ...
                        # Occasional slot state change (available → occupied or
                        # vice versa)
                        prev = (
                            "available"
                            if reading["occupancy_rate"] > 0.5
                            else "occupied"
                        )
                        new = (
                            "occupied" if prev == "available" else "available"
                        )
                        ssl = SlotStateLog(
        ...
        session.commit()
        logger.info(
            "seed: day %d/%d — %d occupancy records",
            day_offset,
            days,
            occ_count,
        )
        ...
        for lot_id, info in lot_map.items():
            ...
            for si in range(n_sessions):
                ...
                final_price = round(
                    entry_price * (0.8 + 0.4 * _seed_rng.random()), 2
                )
                ...
                ps = ParkingSession(
                    ...
                    payment_method=_seed_rng.choice(
                        ["card", "card", "card", "cash"]
                    ),
                )
                session.add(ps)
        ...
        for lot_id, info in lot_map.items():
            for hour in range(6, 22, 2):
                ...
                actual_occ = (
                    info["occ_pct"]
                    / 100
                    * _occupancy_diurnal(hour, info["profile"])
                )
                ...
                pm = PredictionMetric(
                    ...
                    mae=mae,
                    model_version=_seed_rng.choice(
                        ["rf+xgb_ensemble_v2", "rf+xgb_ensemble_v3"]
                    ),
                    timestamp=ts,
                )
                session.add(pm)
        ...
        for lot_id, info in lot_map.items():
            occ_records = [
                r
                for r in all_occupancy_rows
                if r[0] == lot_id and r[1].date() == day_date
            ]
            if occ_records:
                avg_occ = round(
                    sum(r[2]["occupancy_rate"] for r in occ_records)
                    / len(occ_records),
                    4,
                )
                total_day_rev = round(
                ...
                rr = RevenueRecord(
                    ...
                    total_transactions=day_sessions
                    * 2,  # deposit + fee per session
                ...
                payload=json.dumps(
                    {
                        ...
                        "total_revenue": round(
                            ...
                            * 0.02,
                            2,
                        ),  # ~2% of theoretical daily max
                    }
                ),
                status=OUTBOX_DELIVERED,
                created_at=ts,
                processed_at=ts
                + timedelta(seconds=_seed_rng.randint(10, 120)),
            )
            session.add(lo)
    ...
    for lot_id, info in lot_map.items():
        ...
        for pi in range(3):  # 3 prebookings per lot
            ...
            target = target_d.replace(
                hour=target_h, minute=0, second=0, microsecond=0
            )
            slot_entry = _seed_rng.choice(slot_lot)
            pbid = f"PB-{lot_id}-{pi}-{
                hashlib.sha256(str(_seed_rng.random()).encode()).hexdigest()[
                    :6
                ]
            }"
