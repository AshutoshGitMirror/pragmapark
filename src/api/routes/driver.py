...
from src.api.database import get_session as db_session, get_latest_occupancies, lot_to_summary, ParkingLot, OccupancyRecord, MicroSlot
...
def _batch_slot_type_counts(db, lot_ids: list) -> dict:
...
async def search_lots(offset: int = Query(0, ge=0, description="Number of records to skip"),
                      limit: int = Query(100, ge=1, le=1000, description="Max records to return"),
                      slot_type: str = Query(None, description="Filter by slot type"),
                      max_price: Optional[float] = Query(None, ge=0, description="Max hourly price"),
...
async def lot_detail(lot_id: str = Path(..., pattern=r"^[a-zA-Z0-9_-]{1,50}$"), user: dict = Depends(get_current_user)):
    db = db_session()
    try:
        ...
        enriched = pipeline.driver_search_lots([{
            ...
            "current_occupancy": occ, "current_price": cur_price,
        ...
        return DriverLotDetail(
            ...
            predicted_occupancy=prediction.get("predicted_occupancy", round(occ, 3)),
            current_price=prediction.get("dynamic_price", cur_price),
            available_spots=prediction.get("available_spots",
                max(0, int(lot.total_slots * (1 - occ)))),
            available_handicap=sc["handicap"],
            available_ev=sc["ev"],
            available_regular=sc["regular"],
            recent_occupancy=[
