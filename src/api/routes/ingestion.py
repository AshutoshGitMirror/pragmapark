...
async def ingest_sensor_readings(
    ...
    if body.ultrasonic_readings is None or body.vision_readings is None:
        ...
        if len(body.ultrasonic_readings) != len(body.vision_readings):
            raise HTTPException(
                400,
                "ultrasonic_readings and vision_readings "
    ...
    db.commit()
    logger.info(
        "Fused sensor readings for lot %s: occupancy=%.1f%% fp_rate=%.2f",
        body.lot_id,
        occ_rate * 100,
        fp_rate,
    )
...
async def ingest_occupancy(
    report: IngestOccupancyRequest,
    user: dict = Depends(get_current_user),
    db=Depends(get_db),
):
    require_role(user, {"admin", "city_planner", "lot_owner", "sensor"})
    logger.warning(
        "event=ingestion.aggregated fusion=bypassed lot=%s — "
        "use POST /ingestion/sensor-readings for "
        "dual-sensor fusion pipeline",
        report.lot_id,
    )
    try:
        lot = (
            db.query(ParkingLot)
            .filter(ParkingLot.lot_id == report.lot_id)
            .first()
        )
        if not lot:
        ...
        if user.get("role") != "admin":
            caller = (
                db.query(UserModel)
                .filter(UserModel.email == user.get("sub"))
                .first()
            )
        ...
        latest = (
            db.query(OccupancyRecord)
            .filter(OccupancyRecord.lot_id == report.lot_id)
            .order_by(OccupancyRecord.timestamp.desc())
            .first()
        )
        if latest:
            ...
            logger.info(
                "No prior occupancy for lot %s, "
                "using base_price=%.2f as fallback",
                report.lot_id,
                float(lot.base_price),
            )
        record = OccupancyRecord(
        ...
        db.commit()
        logger.info(
            "Ingested occupancy for lot %s: %.1f%%",
            report.lot_id,
            occ_rate * 100,
        )
        return IngestOccupancyResponse(
            status="ingested", lot_id=report.lot_id, occupancy_rate=occ_rate
        )
