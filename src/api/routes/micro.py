...
from src.api.database import get_db, ParkingLot, MicroSlot, MicroZone, SlotReservation, PrebookRecord
from src.api.auth import get_current_user
from src.api.utils import require_admin, RateLimiter, driver_id
...
from src.constants import PREBOOK_SCORE_PROB_WEIGHT, PREBOOK_SCORE_PRICE_PENALTY, PREBOOK_DEFAULT_PRIORITY, LAYER_NAMES, SLOT_TYPE_REGULAR_MAX, SLOT_TYPE_HANDICAP_MAX, SLOT_TYPE_EV_MAX, SLOT_TYPE_COVERED_MAX
from src.micro.state_engine import PREBOOK_GRACE_S
...
def _slots_to_response(slots: list, lot: ParkingLot, modifiers: list[float]) -> list[SlotResponse]:
...
def _rank_slots(db, slots: list[PrebookSlotItem], lot: ParkingLot, modifiers: list[float], driver_id: str) -> list[dict[str, Any]]:
    base_price = float(lot.base_price)
    scored = []
    for item in slots:
        db_slot = _find_slot(db, lot.lot_id, item.slot_index)
        if not db_slot:
            continue
        ...
        scored.append(dict(
            ...
            priority=item.priority if item.priority is not None else PREBOOK_DEFAULT_PRIORITY,
        ))
...
async def reserve_slot(body: ReserveSlotRequest, user: dict = Depends(get_current_user), db = Depends(get_db)):
    did = driver_id(user)
    slot_id = None
    try:
        slot = db.query(MicroSlot).filter(
...
async def prebook_slot(body: PrebookRequest, user: dict = Depends(get_current_user), db = Depends(get_db)):
    did = driver_id(user)
    ...
    try:
        ...
        prebook_record = PrebookRecord(
            prebook_id=prebook_id, lot_id=body.lot_id, driver_id=did,
...
def _find_fallback_slot(db, prebook, did):
    fallback_order = db.query(PrebookRecord).filter(
        PrebookRecord.driver_id == did, PrebookRecord.lot_id == prebook.lot_id,
...
async def confirm_prebook(body: ConfirmPrebookRequest, user: dict = Depends(get_current_user), db = Depends(get_db)):
    did = driver_id(user)
    try:
        prebook = db.query(PrebookRecord).filter(
            PrebookRecord.prebook_id == body.prebook_id, PrebookRecord.driver_id == did,
        ).first()
        if not prebook:
        ...
        result = create_session(
            lot_id=prebook.lot_id, slot=prebook.slot_index, driver_id=did,
        )
        slot = db.query(MicroSlot).filter(MicroSlot.id == prebook.slot_id).first()
        return ConfirmPrebookResponse(
            session_id=result["session_id"], prebook_id=prebook.prebook_id,
...
async def release_slot(body: ReleaseSlotRequest, user: dict = Depends(get_current_user), db = Depends(get_db)):
    ...
    try:
        res = db.query(SlotReservation).filter(
            SlotReservation.id == body.reservation_id, SlotReservation.driver_id == did,
        ).first()
        if not res:
