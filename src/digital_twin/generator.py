...
class Generator:
    ...
    def encode(
        self, x: np.ndarray, conditions: np.ndarray
    ...
    def _critic_gradient_penalty(
        self, real: np.ndarray, fake: np.ndarray, conditions: np.ndarray
    ) -> float:
        ...
        # d_xc is (batch, input_dim); we only want gradient w.r.t state (first
        # state_dim cols)
    ...
    def train_step(
        ...
        if real_samples.shape[1] < self.state_dim:
            padding = np.zeros(
                (batch_size, self.state_dim - real_samples.shape[1])
            )
        ...
        if conditions is None:
            cond_indices = np.random.randint(
                0, self.num_scenarios, size=batch_size
            )
        ...
        d_logvar_kl = (self.kl_weight * (np.exp(log_var) - 1)) / (
            2 * batch_size * self.latent_dim
        )
        ...
        self.W_logvar = self._adam_update(
            "W_logvar", self.W_logvar, dW_logvar, lr
        )
        self.b_logvar = self._adam_update(
            "b_logvar", self.b_logvar, db_logvar, lr
        )
    ...
    def wgan_train_step(
        ...
        """Alternating WGAN-GP training: critic n_critic times, generator once.
        ...
        for _ in range(self.n_critic):
            ...
            # d_wasserstein / d_params = d(mean(critic(fake)))/d_params -
            # d(mean(critic(real)))/d_params

            # Gradient through critic for real samples
            # critic(real) = forward pass, we need
            # d(mean(critic(real)))/d(W_d*, b_d*)
            ...
            # === Gradient penalty gradient (approx via stop-gradient) ===
            # For a full gradient, we'd need 2nd-order gradients through
            # critic which is complex in numpy. We approximate by computing
            # GP contribution through the interpolated path. This is a
            # standard WGAN-GP training signal even without full hessian.
            #
            # We re-use the interpolated forward pass from
            # _critic_gradient_penalty
            alpha = np.random.rand(batch_size, 1)
            ...
            self.W_d1 = self._adam_update(
                "W_d1", self.W_d1, dW_d1_total, lr_critic
            )
            self.b_d1 = self._adam_update(
                "b_d1", self.b_d1, db_d1_total, lr_critic
            )
            self.W_d2 = self._adam_update(
                "W_d2", self.W_d2, dW_d2_total, lr_critic
            )
            self.b_d2 = self._adam_update(
                "b_d2", self.b_d2, db_d2_total, lr_critic
            )
            self.W_d3 = self._adam_update(
                "W_d3", self.W_d3, dW_d3_total, lr_critic
            )
            self.b_d3 = self._adam_update(
                "b_d3", self.b_d3, db_d3_total, lr_critic
            )

            # Regenerate fake for next critic iteration (generator hasn't
            # changed yet)
            z = np.random.randn(batch_size, self.latent_dim)
            fake = self.forward(z, conditions)
        ...
        avg_critic_loss = (
            float(np.mean(critic_loss_vals)) if critic_loss_vals else 0.0
        )
        gen_loss_val = (
            float(gen_loss)
            if isinstance(gen_loss, np.ndarray)
            else float(gen_loss)
        )
    ...
    def train(
        self, real_data: np.ndarray, epochs: int = 500, wgan_epochs: int = 0
    ) -> List[float]:
        ...
        for ep in range(epochs):
            idx = np.random.choice(
                len(real_data), min(32, len(real_data)), replace=False
            )
            batch = real_data[idx]
            ...
            if (ep + 1) % 100 == 0:
                losses.append(loss)
                logger.info(
                    "Generator epoch %d/%d: loss=%.4f", ep + 1, epochs, loss
                )
        self.trained = True
        ...
        if wgan_epochs > 0:
            ...
            for ep in range(wgan_epochs):
                ...
                if (ep + 1) % 50 == 0:
                    logger.info(
                        "WGAN epoch %d/%d: critic=%.4f gen=%.4f gp=%.4f",
                        ep + 1,
                        wgan_epochs,
                        result["critic_loss"],
                        result["gen_loss"],
    ...
    def online_update(
        ...
        if len(self._online_buffer) >= batch_size:
            ...
            # CVAE update
            cvae_loss = self.train_step(
                batch, lr=learning_rate, conditions=null_cond
            )
            self._online_steps += 1

            # WGAN update (every other online step) — alternates to save
            # compute
            wgan_metrics = {}
            if self._online_steps % 2 == 0:
                wgan_metrics = self.wgan_train_step(
                    batch,
                    conditions=null_cond,
            ...
            self._online_buffer = []
            logger.info(
                "generator.online_update batch=%d cvae=%.6f "
                "wgan=%s total_steps=%d",
                batch_size,
                cvae_loss,
                wgan_metrics.get("gen_loss", "N/A"),
                self._online_steps,
            )
            return {
                "trained": True,
        ...
        return {
            "trained": False,
            "samples": len(self._online_buffer),
            "cvae_loss": None,
            "wgan": None,
            "total_steps": self._online_steps,
        }
