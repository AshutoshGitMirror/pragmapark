...
class ScenarioEngine:
    def __init__(self, generator: Optional[Generator] = None):
        self.generator = (
            generator if generator is not None else Generator(latent_dim=8)
        )
    ...
    def register_defaults(self):
        ...
        def apply_capacity_expansion(s: dict, v: dict) -> dict:
            ...
            occ_diff = v["occupancy_rate"] - s.get(
                "occupancy_rate", DEFAULT_OCCUPANCY
            )
            occupancy = float(
                np.clip(
                    s.get("occupancy_rate", DEFAULT_OCCUPANCY)
        ...
        def apply_holiday_spike(s: dict, v: dict) -> dict:
            ...
            holiday_bump = (HOLIDAY_OCCUPANCY_MULTIPLIER - 1.0) * s.get(
                "occupancy_rate", DEFAULT_OCCUPANCY
            )
            occupancy = float(
                np.clip(
            ...
            price = float(
                np.clip(
                    s.get("price", 10.0)
                    * (1.1 + abs(v["price"] - s.get("price", 10.0)) / 100.0),
                    5.0,
                    200.0,
                )
            )
            congestion = (
                "high"
                if occupancy > HOLIDAY_CONGESTION_THRESHOLD
                else "moderate"
            )
            return {
                **s,
