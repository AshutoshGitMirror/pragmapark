...
class PricingController:
    ...
    def _get_marl_multiplier(self, occupancy: float, current_price: float,
                              zone_idx: int) -> float:
    ...
    def get_price(self, occupancy: float, current_price: float,
        ...
        if self.marl and zone_id is not None:
            ...
            multiplier = self._get_marl_multiplier(occupancy, current_price, zone_idx)
            source = "marl"
        elif self.agent:
            state = np.array([occupancy, current_price, 0.5])
        ...
        if self.marl and zone_id is not None:
            logger.debug("event=pricing.marl zone=%s occupancy=%.3f multiplier=%.4f new_price=%.2f",
                         zone_id, occupancy, multiplier, new_price)
