...
class _ModelCompat:
    """Backward-compatible wrapper so agent.model.predict(x) works.

    Used by multi_agent.py:_compute_agent_qs() and
    train_control.py:train_neural_control().
...
class NeuralAgent:
    ...
    def __init__(self, state_size: int = 3, action_size: int = 1):
        ...
        self.W1 = np.random.randn(self.input_dim, self.hidden) * np.sqrt(
            2.0 / self.input_dim
        )
        self.b1 = np.zeros(self.hidden)
        self.W2 = np.random.randn(self.hidden, self.hidden) * np.sqrt(
            2.0 / self.hidden
        )
    ...
    def _scale_state(self, state: np.ndarray) -> np.ndarray:
        s = (
            ...
            else np.asarray(state).flatten().copy()
        )
        s[1] = s[1] / 50.0  # Normalize price to ~[0, 1]
        return s

    def _predict_q(
        self, params: dict, scaled_s: np.ndarray, action: float
    ) -> float:
    ...
    def _max_q(
        self, params: dict, scaled_ns: np.ndarray, n_candidates: int = 10
    ) -> float:
    ...
    def act(self, state: np.ndarray, train: bool = True) -> float:
        ...
        # Greedy action: batch-evaluate 10 candidate actions (whitepaper:
    ...
    def train(
        ...
        next_state = (
            next_state.flatten()
            if hasattr(next_state, "ndim") and next_state.ndim > 1
            else np.asarray(next_state).flatten()
        )
        ...
        batch = random.sample(
            list(self.memory), min(len(self.memory), self.batch_size)
        )
        ...
        for s, a, r, ns, d in batch:
            ...
            if d:
                ...
                # Bootstrap from target network: r + γ · max_a' Q_target(s',
                # a')
                target = r + self.gamma * self._max_q(
                    self._params(target=True), scaled_ns
                )
            else:
                target = r
    ...
    def model(self, _value):
        # Ignore direct assignment (legacy MLPRegressor initialization paths)
        pass
