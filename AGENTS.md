...
- **Deploy flow:** push `main` → CI runs. `lint`/`test`/`e2e`/`security` (bandit) jobs ALL PASS (post A114 — the src/ bandit findings are fixed), so `checksPass` goes green and Render **auto-deploys** (`autoDeploy=yes`, `autoDeployTrigger=checksPass` on srv `srv-d8bvbuv7f7vs73cs0tu0`, confirmed via API). Historically deploys looked manual (504301f, e1a4f01, b813ce1) ONLY because the failing bandit job kept `checksPass` red. `render.yaml` has NO `autoDeploy` key, so the dashboard setting governs. **CAVEAT (2026-07-19, A122):** auto-deploy did NOT fire on push for SHAs `4a6c0da` and `c4eeb7c` (no build was created) — `render_trigger_deploy` was used as the fallback (`dep-d9edn93tqb8s73aaips0`, `dep-d9edqsv7f7vs73ae37dg`). So if a push leaves the service without a build, a manual trigger is acceptable. The live service buildCommand is `pip install -r requirements.txt` (NO SPA build — see A116 CORRECTION), so the committed `frontend/dist` is served: rebuild+commit dist is mandatory per frontend change.
...
- **A116 CORRECTION (2026-07-19, VERIFIED via Render API):** the **live** Render service (`srv-d8bvbuv7f7vs73cs0tu0`) buildCommand is `pip install -r requirements.txt` ONLY — it does **NOT** build the frontend (no `npm run build`). The repo `render.yaml` *describes* a frontend build step, but `render.yaml` is only applied at service creation/sync; the **live service config is authoritative** and serves the committed `frontend/dist`. So the original A116 premise holds: Render serves the committed `frontend/dist`, and every frontend change MUST be rebuilt + committed (the `!frontend/dist/` keep in `.gitignore` ensures it stays tracked). Rebuilding+committing `dist` is mandatory, not optional.
...
## 11. Open live-audit findings (FINAL AUDIT 2026-07-19, app v `064154f` — A115 F3 + A116 F1/F6 + A117 F4 all CLOSED, live-verified)
...
- **F4 — session-state inconsistency (seed-data artifact), FIXED A117, LIVE-VERIFIED CLOSED 2026-07-19:** PENDING_SETTLEMENT sessions for seed driver caused Find banner "You already have an active session" vs Parking "Session Ended / Confirm payment" vs dashboard "PAYMENT DUE 0". A117 added admin-only `POST /api/v1/admin/sessions/clear-pending` (settles, not deletes). **Live-verified:** authenticated as admin (fresh session `a117admin`), `POST …/clear-pending` → `{"cleared":62,"status":"settled"}`; then fresh driver session `a117driver` → driver dashboard now shows **"No active session"** + `Last session … ₹0` (was the phantom active-session banner). **CLOSED.**
