...
- **A128** — DIGITAL TWIN TRANSFORMATION (observation-driven, measurable, persistent). Per `.opencode/plans/digital_twin_transformation_plan.md` (P1–P7) the twin was rebuilt from an in-memory prototype into a persistent, auditable subsystem. New durable ORM models in `src/digital_twin/orm.py`: `TwinObservation`/`TwinState`/`TwinForecast`/`TwinScenarioRun`/`TwinModelVersion`; migration `alembic/versions/0021_add_twin_models.py` (head `0021`, down `0020`). `src/digital_twin/service.py` `TwinService`: `ingest_observation`/`generate_forecasts` (anchored to latest REAL observation via `input_observation_id`, horizons `[15,60,1440]` min, stamps model name+version)/`evaluate_forecasts` (matches forecast target to later observed outcome)/`persist_scenario_run` (writes ONLY `TwinScenarioRun`)/`register_model_version`. `src/digital_twin/stid.py` retrains only on real observed occupancy; `spatial.py` uses real geo/road distances (no random embeddings). `src/digital_twin/scenario.py`: deterministic engine, every run `kind="deterministic"`, never auto-actuates. `src/digital_twin/calibration.py` (P5 best-form re-implementation of the removed generative part): deterministic wrap + **empirical quantile/bootstrap band over REAL observed deltas**, `kind="calibrated"` (NEVER `learned`), `experimental=True` when `n_real<20`, OFFLINE-ONLY, versioned, never auto-actuates. `src/digital_twin/generator.py`: CVAE-WGAN kept **offline-only + versioned artifact** (`to_artifact`/`save_artifact`/`load_artifact`); correct CVAE objective (recon MSE + 0.05·KL) + real WGAN (Wasserstein + GP λ=10, 3 critic steps). `scripts/train_twin_generator.py`: NEW offline train+held-out-eval+versioning script; registers `TwinModelVersion(promotion_status="experimental")`. `src/api/routes/twin.py` + `schemas/twin.py`: `/api/v1/twin` (observations, states, forecasts, scenarios deterministic + calibrated); orchestrator `run_digital_twin_scenario` is READ-ONLY recommendation. **8 non-negotiable principles enforced** (no training on simulated values; every forecast traceable to input observation + later outcome; separate state/forecast/sim/control; rule-based = `deterministic` not `learned`; real spatial data only; no unvalidated generative claims; synthetic data ≠ evidence; scenarios never actuate). **P5 Alternative Path BLOCKED**: rebuild CVAE-WGAN as a causal intervention model requires labelled intervention→outcome data, which does NOT exist (`TwinObservation` has no intervention field; scenario labels are one-hot-by-index). Implemented best form = Recommended Path (offline versioned artifact + deterministic calibrated baseline). `digital_twin_paper.tex` rewritten to the honest persistent architecture (DoD #11). Tests: `test_twin_p1/p7/calibration/spatial/forecasting/digital_twin/routes/generator_offline` = **61 pass**. Smoke: `scripts/train_twin_generator.py` trained on 400 seeded real observations, eval on held-out 80, saved `data/twin_generator/smoke_v1.json` (28KB), persisted `TwinModelVersion(promotion_status="experimental")`, and confirmed NO runtime module imports `generator`.
