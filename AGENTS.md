...
- **A116 CORRECTION (2026-07-19):** the premise "render.yaml has no frontend build step" is now STALE — current `render.yaml` buildCommand is `pip install -r requirements.txt && python scripts/download_models.py && cd frontend && npm install && npm run build && cd ..`, i.e. it **builds the SPA fresh on every deploy** (Render's build image includes node/npm). So the committed `frontend/dist` is overwritten on each deploy; a stale committed dist can no longer serve `$`. For belt-and-suspenders this commit STILL rebuilds+commits `dist` (matches the A116 convention), but going forward the deploy is self-correcting.
