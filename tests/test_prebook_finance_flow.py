...
from src.api.database import get_session, User, PrebookRecord, ParkingSession, Transaction
from src.constants import BOOKING_FEE
...
def _create_lot(lot_id="prebook_fin_lot"):
    ...
    try:
        ...
        if not db.query(ParkingLot).filter(ParkingLot.lot_id == lot_id).first():
            db.add(ParkingLot(lot_id=lot_id, name="Finance Test Lot", total_slots=10, base_price=10.0, price_cap=50.0))
            db.commit()
    finally:
        db.close()
...
class TestPrebookFinanceFlow:
    ...
    def setup_driver(self, client):
        ...
        resp = client.post("/api/v1/auth/register", json={
            "email": email, "password": password, "full_name": "Finance Driver",
        })
        ...
        topup_resp = client.post("/api/v1/wallet/topup", json={"amount": 100.0}, headers=headers)
    ...
    def test_prebook_finance_integration_flow(self, client, setup_driver, admin_headers):
        email, headers = setup_driver
        lot_id = "prebook_fin_lot"
        _create_lot(lot_id)

        # Seed micro slots for the lot. Clear cookies first so admin_headers takes priority.
        client.cookies.clear()
        seed_resp = client.post(f"/api/v1/micro/lots/{lot_id}/slots/seed", headers=admin_headers)
        ...
        prebook_resp = client.post("/api/v1/micro/prebook", json={
            "lot_id": lot_id,
        ...
        try:
            ...
            pb_rec = db.query(PrebookRecord).filter(PrebookRecord.prebook_id == prebook_id).first()
            ...
            fee_tx = db.query(Transaction).filter(Transaction.tx_hash == f"fee_{prebook_id}").first()
            assert fee_tx is not None
            assert fee_tx.amount == 2.0
            assert fee_tx.action == "booking_fee"

            dep_tx = db.query(Transaction).filter(Transaction.tx_hash == f"deposit_{prebook_id}").first()
        ...
        client.cookies.clear()
        confirm_resp = client.post("/api/v1/micro/confirm", json={
        ...
        try:
            ...
            sess = db.query(ParkingSession).filter(ParkingSession.session_id == session_id).first()
            assert sess is not None
            assert sess.status == "running"

            # Calculate exactly how many hours to subtract to get exactly $6.0 charge
            entry_price = float(sess.entry_price)
            hours_to_subtract = 6.0 / entry_price

            # 4. Modify session start_time without buffer (delay increases time slightly, rounding up to $6.0)
            sess.start_time = datetime.now(timezone.utc) - timedelta(hours=hours_to_subtract)
            db.commit()
        finally:
            db.close()
        ...
        # End the session.
        client.cookies.clear()
        end_resp = client.post("/api/v1/sessions/end", json={
        ...
        try:
            ...
            refund_tx = db.query(Transaction).filter(
