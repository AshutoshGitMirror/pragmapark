#!/usr/bin/env python3
"""THE PEOPLE vs. PRAGMA SMART PARKING — 10 personas, 10 nightmares, real API calls."""

import os, sys, uuid, random, math, time, threading, json
...
from src.api.database import get_engine, get_session, ParkingLot, MicroSlot, User, ParkingSession, RevenueRecord
...
try:
    if not db.query(User).filter(User.email == "admin@people.io").first():
        db.add(User(email="admin@people.io", hashed_password=hash_password("Admin123!"),
                    full_name="Admin", role="admin"))
    for info in [("lot_a", 200, 10.0), ("lot_b", 500, 12.0), ("lot_c", 100, 8.0), ("lot_d", 300, 7.0)]:
        if not db.query(ParkingLot).filter(ParkingLot.lot_id == info[0]).first():
            db.add(ParkingLot(lot_id=info[0], name=f"Pragma {info[0].upper()}",
    ...
    db.commit()
    for lot_id, total, _ in [("lot_a", 200, 10.0), ("lot_b", 500, 12.0), ("lot_c", 100, 8.0), ("lot_d", 300, 7.0)]:
...
ALL = lambda *c: any(r.status_code in (c if isinstance(c, (list,tuple)) else [c]) for c in (c,))

def login(email, pw="Pass123!"):
    r = client.post("/api/v1/auth/login", json={"email": email, "password": pw})
    if r.status_code != 200:
        r2 = client.post("/api/v1/auth/register", json={"email":email, "password":pw, "full_name":email.split("@")[0]})
        if r2.status_code in (200, 400):
            r = client.post("/api/v1/auth/login", json={"email": email, "password": pw})
    return r.json()["access_token"]

from src.api.server import _global_rate_limiter
from src.api.routes.auth import _login_ip_limiter, _login_account_limiter, _register_limiter
...
d = r.json()
print(f"   📍 Lot A: ${d['current_price']}/hr, {d['available_spots']} spots, pred={d['predicted_occupancy']}")
assert d["latitude"] is not None

# Start session
r = client.post("/api/v1/sessions/start", json={"lot_id":"lot_a","slot":1}, headers=h)
...
r = client.post("/api/v1/micro/prebook", json={
    "lot_id":"lot_a","slots":[{"slot_index":2,"priority":1}],
    "target_time":(datetime.now(timezone.utc)+timedelta(minutes=5)).isoformat(),
}, headers=h)
if r.status_code == 200:
...
js = r.json()
print(f"   💳 ${js['amount_charged']:.2f} (entry=${js['entry_price']:.2f} → final=${js['final_price']:.2f})")
...
sep()
print("""💰 2. BARGAIN BOB
    \"$10 vs $12? that's $0.20 in sales tax! i'll drive 15 min to save $0.15.\" """)
...
r = client.get("/api/v1/driver/lots", headers=h)
lots = r.json()["lots"]
prices = {l["lot_id"]: l["dynamic_price"] for l in lots}
cheapest = min(prices, key=prices.get)
print(f"   💸 Bob scans prices: {prices}")
print(f"   🏆 Cheapest: {cheapest} @ ${prices[cheapest]:.2f}/hr 'still more than i'd like'")

# Start session on the cheapest
r = client.post("/api/v1/sessions/start", json={"lot_id":cheapest,"slot":1}, headers=h)
assert r.status_code == 200, f"start: {r.text}"
sid = r.json()["session_id"]
...
lots_admin = r.json()
lot_data = next((l for l in lots_admin if l["lot_id"] == cheapest), None)
if lot_data:
    print(f"   📊 Base=${lot_data['base_price']:.2f}, cap=${lot_data['price_cap']:.2f}. Bob: 'hmm, fine.'")

# Bob prebooks 3 slots in different lots (gaming the system)
for idx, lid in enumerate(["lot_a","lot_b","lot_c"]):
    r = client.post("/api/v1/micro/prebook", json={
        "lot_id":lid, "slots":[{"slot_index":idx+1,"priority":1}],
        "target_time":(datetime.now(timezone.utc)+timedelta(hours=1)).isoformat(),
    }, headers=h)
    print(f"   📅 Bob prebooks {lid} slot {idx+1}: {'👍' if r.status_code==200 else f'status={r.status_code}'}")

# End session
r = client.post("/api/v1/sessions/end", json={"session_id":sid}, headers=h)
if r.status_code == 200:
    print(f"   💳 Paid ${r.json()['amount_charged']:.2f}. Bob: 'i could've parked on the street for FREE'")
print("✅ DONE\n")
...
results = []
def mash_start():
    r = client.post("/api/v1/sessions/start", json={"lot_id":"lot_a","slot":3}, headers=h)
...
if sid:
    r = client.post("/api/v1/sessions/end", json={"session_id":sid}, headers=h)
    if r.status_code == 200:
        print(f"   💳 ${r.json()['amount_charged']:.2f} Larry: 'FINALLY. also you charged me WHAT.'")
    # Try ending twice — classic panic
    r2 = client.post("/api/v1/sessions/end", json={"session_id":sid}, headers=h)
    print(f"   🔄 End twice: status={r2.status_code} (should be 404): {r2.text[:50]}")
...
# ═══════════════════════════════════════════════════════════════════════════
sep()
print("""💇 4. KAREN CONNORS
    \"I was CHARGED $2.25 but the SIGN SAID $2.00. I WANT TO SPEAK TO THE MANAGER.\" """)
...
r = client.get("/api/v1/driver/lots/lot_a", headers=h)
d = r.json()
print(f"   🔬 Karen inspects Lot A: base=${d['base_price']:.2f}, current=${d['current_price']:.2f}")
print(f"   🔬 avail={d['available_spots']}, pred_occ={d['predicted_occupancy']}")

# Start & end session to get a bill
r = client.post("/api/v1/sessions/start", json={"lot_id":"lot_a","slot":4}, headers=h)
assert r.status_code == 200
sid = r.json()["session_id"]
r = client.post("/api/v1/sessions/end", json={"session_id":sid}, headers=h)
assert r.status_code == 200
js = r.json()
print(f"   🧾 Karen's bill: ${js['amount_charged']:.2f} for {js['duration_minutes']} min")
...
if r.status_code == 200:
    pjs = r.json()
    print(f"   💳 Payment: ${pjs['amount']:.2f}, tx={pjs['tx_hash'][:12]}... bc_ref={pjs['blockchain_ref'][:12]}...")
...
# ═══════════════════════════════════════════════════════════════════════════
sep()
print("""🌿 5. STONER STEVE
    \"dude... i parked... somewhere. i think row C? or was it row E? whatever bro.\" """)

h = auth(f"steve-{os.getpid()}@people.io")

# Start session — Steve parks
r = client.post("/api/v1/sessions/start", json={"lot_id":"lot_b","slot":5}, headers=h)
assert r.status_code == 200
sid = r.json()["session_id"]
...
if r.status_code == 200:
    slots = r.json()
    print(f"   🅿️  Slots on lot_b: {slots.get('total', '?')} total, {slots.get('available', '?')} avail")
...
r = client.post("/api/v1/micro/prebook", json={
    "lot_id":"lot_b", "slots":[{"slot_index":5,"priority":1}],
    "target_time":(datetime.now(timezone.utc)+timedelta(hours=1)).isoformat(),
}, headers=h)
if r.status_code == 200:
    print(f"   🤦 Steve prebooks slot 5... while sitting in slot 5. 'for later, dude.'")
elif r.status_code == 409:
    print(f"   🤦 Steve prebook blocked (already occupied): 409 — makes sense actually")
else:
    print(f"   🤦 Steve's prebook: status={r.status_code}")

# Try starting another session (he forgot he's already parked)
r = client.post("/api/v1/sessions/start", json={"lot_id":"lot_b","slot":6}, headers=h)
...
r = client.post("/api/v1/sessions/start", json={"lot_id":"lot_c","slot":1}, headers=h)
assert r.status_code == 200
sid1 = r.json()["session_id"]
print(f"   🚗 Hannah parks. session={sid1}")

# She rushes to end it (8 min later... well, immediately in test time)
r = client.post("/api/v1/sessions/end", json={"session_id":sid1}, headers=h)
assert r.status_code == 200
js1 = r.json()
min_charge = js1["amount_charged"]
print(f"   💳 Hannah pays ${min_charge:.2f} for {js1['duration_minutes']} min")
print(f"   😤 '${min_charge:.2f} for EIGHT MINUTES?! that's robbery!'")

# She does 4 more quick sessions back-to-back
for i in range(4):
    r = client.post("/api/v1/sessions/start", json={"lot_id":"lot_c","slot":i+2}, headers=h)
    if r.status_code != 200:
        # Maybe another lot
        r = client.post("/api/v1/sessions/start", json={"lot_id":"lot_d","slot":i+1}, headers=h)
    if r.status_code != 200:
        print(f"   ❌ Hannah can't start session {i+2}: {r.status_code}")
        continue
    sn = r.json()["session_id"]
    r = client.post("/api/v1/sessions/end", json={"session_id":sn}, headers=h)
    if r.status_code == 200:
        total = sum(
...
print(f"   📊 Hannah's total: she's mad about the minimum charge structure")
...
for lot_id in ["lot_a", "lot_b", "lot_c", "lot_d"]:
    ...
    if r.status_code == 200:
        ...
        print(f"   📊 {lot_id}: ${d2['current_price']:.2f}/hr, base=${d2['base_price']:.2f}, pred={d2['predicted_occupancy']}")

# Trevor checks revenue endpoint to see if his "strategies" would work
r = client.get("/api/v1/revenue/overview", headers=auth(f"admin-{os.getpid()}@people.io"))
if r.status_code == 200:
    rev = r.json()
    print(f"   💰 Revenue overview (for analysis): {type(rev).__name__} = {str(rev)[:100]}")
else:
    print(f"   💰 Revenue overview: status={r.status_code} (need admin?)")

# Trevor prebooks the CHEAPEST lot for 3 different time slots
cheapest_lot = min(prices_seen, key=prices_seen.get) if prices_seen else "lot_d"
for hours_ahead in [1, 2, 3]:
    r = client.post("/api/v1/micro/prebook", json={
        "lot_id":cheapest_lot, "slots":[{"slot_index":hours_ahead,"priority":1}],
        "target_time":(datetime.now(timezone.utc)+timedelta(hours=hours_ahead)).isoformat(),
    }, headers=h)
    status = "✅" if r.status_code == 200 else f"status={r.status_code}"
    print(f"   📅 Trevor prebooks {cheapest_lot} T+{hours_ahead}h: {status}")
...
best_lot = max(lots, key=lambda l: l["available_spots"])
print(f"   🏃 Maria picks {best_lot['lot_id']} — {best_lot['available_spots']} spots open")
print(f"   'i don't care about the price just park the VAN'")

# Start session — slot near the entrance
r = client.post("/api/v1/sessions/start", json={"lot_id":"lot_a","slot":1}, headers=h)
assert r.status_code == 200, f"'NOTHING WORKS': {r.text}"
sid = r.json()["session_id"]
print(f"   ✅ 'FINALLY. OK kids out of the car. NO don't touch that.'")
...
if r.status_code == 200:
    ...
    print(f"   ♿  {len(handicap)} handicap slots. Maria: 'i need one for the STROLLER not me!'")

# End session (30 min later — got the kids, just picking up)
r = client.post("/api/v1/sessions/end", json={"session_id":sid}, headers=h)
assert r.status_code == 200, f"end: {r.text}"
js = r.json()
print(f"   💳 Maria: '${js['amount_charged']:.2f}?! i was here for 10 MINUTES!'")
...
# ═══════════════════════════════════════════════════════════════════════════
sep()
print("""👴 9. GRANDPA GEORGE
    \"Back in MY DAY you pulled into a spot and that was it.
    What's this 'pre-book' nonsense? And why does my phone need my location?\" """)

h = auth(f"george-{os.getpid()}@people.io")

# George doesn't understand the app
print("   📱 George: 'how do i... no that's not... WHERE'S THE PARKING BUTTON?'")
...
r = client.get("/api/v1/driver/lots", headers=h)
if r.status_code == 200:
    print(f"   📋 George: 'whatever, just show me a spot'")
...
r = client.post("/api/v1/sessions/start", json={"lot_id":"lot_a","slot":7}, headers=h)
assert r.status_code == 200
sid = r.json()["session_id"]
print(f"   🚗 George: 'finally parked. took 3 screens. IN MY DAY...'")

# George tries to prebook by accident (taps wrong button)
r = client.post("/api/v1/micro/prebook", json={
    "lot_id":"lot_a", "slots":[{"slot_index":7}],
...
r = client.post("/api/v1/sessions/end", json={"session_id":sid}, headers=h)
if r.status_code == 200:
    print(f"   💳 George: 'so it just... charges me? automatically?' ${r.json()['amount_charged']:.2f}")
    print(f"   🔗 blockchain: {r.json()['blockchain_ref'][:12]}... 'a WHAT chain?'")
print("""   💬 George: 'this newfangled block chain thing... in my day we used CASH.
...
if r.status_code == 200:
    for lot in r.json():
        print(f"   🅿️  {lot['lot_id']}: ${lot['base_price']:.2f} base, {lot['total_slots']} slots")
...
for i in range(3):
    r = client.post("/api/v1/sessions/start", json={"lot_id":"lot_a","slot":10+i}, headers=ha)
    if r.status_code == 200:
        sid = r.json()["session_id"]
        r = client.post("/api/v1/sessions/end", json={"session_id":sid}, headers=ha)
        if r.status_code == 200:
            print(f"   💰 Andy generates $" + str(r.json()["amount_charged"])[:6])
...
if r.status_code == 200:
    print(f"   ✏️  Andy updates lot_a pricing: base=$15, cap=$100")
else:
...
if r.status_code == 200:
    ...
    print(f"   ⛏️  No pending transactions (that's fine, Andy)")
r = client.get("/api/v1/blockchain/chain", headers=h_admin)
if r.status_code == 200:
    chain = r.json()
    print(f"   🔗 Blockchain: {len(chain) if isinstance(chain, list) else 'ok'} blocks")
print("""   💬 Andy: 'i want to see LIVE revenue dashboards. TWELVE-FOOT SCREENS.
   this app needs REAL-TIME blockchain settlement. and bigger numbers.'
