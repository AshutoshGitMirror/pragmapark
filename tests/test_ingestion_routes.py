...
class TestIngestOccupancy:
    ...
    def test_ingest_requires_auth(self, client):
        resp = client.post(
            ...
            json={"lot_id": "x", "occupied_slots": 50, "total_slots": 100},
        )
    ...
    def test_ingest_creates_record(self, client, admin_headers):
        ...
        try:
            recs = (
                ...
                .filter(OccupancyRecord.lot_id == "ingest_lot2")
                .all()
            )
            assert len(recs) >= 1
    ...
    def test_ingest_sensor_readings_requires_auth(self, client):
        resp = client.post(
            "/api/v1/ingestion/sensor-readings", json={"lot_id": "ingest_lot"}
        )
        assert resp.status_code in (401, 403)
    ...
    def test_ingest_sensor_readings_simulator_fallback(
        self, client, admin_headers
    ):
        self._create_lot("sim_lot")
        payload = {"lot_id": "sim_lot", "total_slots": 10}
        resp = client.post(
            "/api/v1/ingestion/sensor-readings",
            json=payload,
            headers=admin_headers,
