...
import os, sys, time, threading, json, math, random, gc
from datetime import datetime, timezone, timedelta

sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

os.environ.setdefault("DATABASE_URL", f"sqlite:////tmp/stress_{os.getpid()}.db")
...
from src.api.database import Base, get_engine, get_session, User, ParkingLot, MicroSlot
...
try:
    if not db.query(User).filter(User.email == "admin@str.ess").first():
        db.add(User(email="admin@str.ess", hashed_password=hash_password("Admin123!"),
                    full_name="Stress Admin", role="admin"))
    # create 3 lots
    for info in [("stress_lot_a", 200, 10.0),
                 ("stress_lot_b", 500, 12.0),
                 ("stress_lot_c", 100, 8.0)]:
        if not db.query(ParkingLot).filter(ParkingLot.lot_id == info[0]).first():
            db.add(ParkingLot(lot_id=info[0], name=f"Stress {info[0]}",
...
with get_session() as s:
    for lot_id, total, base_p in [("stress_lot_a", 200, 10.0),
                                   ("stress_lot_b", 500, 12.0),
                                   ("stress_lot_c", 100, 8.0)]:
        if s.query(MicroSlot).filter(MicroSlot.lot_id == lot_id).count() == 0:
            ...
            for r in range(rows):
                ...
                for p in range(per_row):
                    ...
                    roll = random.random()
                    st = "regular"
                    if roll < 0.05: st = "handicap"
                    elif roll < 0.10: st = "ev"
                    elif roll < 0.25: st = "covered"
                    elif roll < 0.30: st = "premium"
                    s.add(MicroSlot(lot_id=lot_id, slot_index=created+1,
                        row_label=rl, position=p+1, slot_type=st, active=1,
        ...
        with _TOKENS_LOCK:
            if _ADMIN_TOKEN_CACHE["token"] is None:
                ...
                if r.status_code != 200:
                    r = _login("admin@str.ess", "Admin123!")
                if r.status_code == 200:
                    _ADMIN_TOKEN_CACHE["token"] = r.json().get("access_token", "")
                else:
                    ...
                    if r.status_code == 200:
                        _ADMIN_TOKEN_CACHE["token"] = r.json().get("access_token", "")
    return {"Authorization": f"Bearer {_ADMIN_TOKEN_CACHE['token']}"} if _ADMIN_TOKEN_CACHE["token"] else {}
...
_target = lambda: (datetime.now(timezone.utc) + timedelta(hours=1)).isoformat()
...
print(f"\n{'='*70}")
print(f"  QA STRESS TEST — 30 iterations × full-system chaos")
print(f"{'='*70}\n")
t0 = time.time()
...
for iteration in range(1, 31):
    ...
    try:
        ...
        check(f"{it_dev}A02 browse lots", r.status_code in (200, 304), r.text[:80])
        lots = r.json() if r.status_code == 200 else []
        check(f"{it_dev}A03 lots list", isinstance(lots, list) and len(lots) > 0, str(lots)[:80])

        # List slots lot_a
        r = client.get("/api/v1/micro/lots/stress_lot_a/slots?limit=50", headers=h)
        check(f"{it_dev}A04 list slots", r.status_code == 200, r.text[:80])
        slots_data = r.json().get("slots", [])
        check(f"{it_dev}A05 slots returned", len(slots_data) > 0, str(len(slots_data)))

        # Slot probability
        if slots_data:
            si = slots_data[0]["slot_index"]
            r = client.get(f"/api/v1/micro/lots/stress_lot_a/slots/{si}/probability?target_time={_target()}", headers=h)
            check(f"{it_dev}A06 slot prob", r.status_code == 200, r.text[:80])
            d = r.json()
            check(f"{it_dev}A07 prob [0,1]", 0 <= d["probability"] <= 1, str(d["probability"]))
            check(f"{it_dev}A08 adj price >0", d["current_price"] > 0, str(d["current_price"]))
        ...
        r = client.post("/api/v1/micro/prebook", json={
            "lot_id": "stress_lot_a",
            "slots": [{"slot_index": idx1, "priority": 1},
        ...
        if r.status_code == 200:
            pb = r.json()
            pid = pb["prebook_id"]
            check(f"{it_dev}A13 prebook active", pb["status"] == "active", pb.get("status", ""))
            check(f"{it_dev}A14 assigned idx", pb["assigned_slot_index"] in (idx1, idx2), str(pb))
            check(f"{it_dev}A15 fallback set", pb.get("fallback_order") is not None or pb.get("assigned_slot_index") == idx2, str(pb))
            check(f"{it_dev}A16 prob > 0", pb["probability"] > 0, str(pb))
            check(f"{it_dev}A17 price > 0", pb["price_at_booking"] > 0, str(pb))

            # Confirm prebook
            r = client.post("/api/v1/micro/confirm", json={"prebook_id": pid}, headers=h)
            if r.status_code == 200:
                cf = r.json()
                check(f"{it_dev}A18 confirm ok", cf["status"] == "confirmed", cf.get("status", ""))
                check(f"{it_dev}A19 has session", bool(cf.get("session_id")), str(cf))
                check(f"{it_dev}A20 slot match", cf.get("slot_index") in (idx1, idx2), str(cf))

                sid = cf["session_id"]

                # Check active session
                r = client.get(f"/api/v1/sessions/active/stress_lot_a", headers=h)
                ...
                if r.status_code == 200:
                    ...
                    check(f"{it_dev}A23 history has session", sess_count >= 1, str(sess_count))

                # End session
                r = client.post("/api/v1/sessions/end", json={"session_id": sid}, headers=h)
                _flow_assert(f"{it_dev}A24 end session", r, 200)
                if r.status_code == 200:
                    ed = r.json()
                    check(f"{it_dev}A25 duration > 0", ed.get("duration_hours", 0) >= 0, str(ed))
                    check(f"{it_dev}A26 amount > 0", ed.get("amount_charged", 0) > 0, str(ed))

                # Confirm payment
                r = client.post("/api/v1/payments/confirm", json={"session_id": sid}, headers=h)
                _flow_assert(f"{it_dev}A27 pay confirm", r, (200, 400, 409))
                if r.status_code == 200:
                    pm = r.json()
                    check(f"{it_dev}A28 tx generated", bool(pm.get("tx_hash")), str(pm))
                    check(f"{it_dev}A29 amount > 0", pm.get("amount", 0) > 0, str(pm))
                ...
                if r.status_code == 200:
                    ...
                    check(f"{it_dev}A31 hist has payment", pay_cnt >= 1, str(pay_cnt))

                # Double pay protection
                r = client.post("/api/v1/payments/confirm", json={"session_id": sid}, headers=h)
                if r.status_code == 200:
                    check(f"{it_dev}A32 double-pay protected", r.json().get("already_paid", False), r.text[:80])

            elif r.status_code == 410:
                check(f"{it_dev}A18 prebook expired (target in past?)", True, "")
            else:
    ...
    try:
        ...
        protected_endpoints = [
            ...
            ("micro_slots", "GET", "/api/v1/micro/lots/stress_lot_a/slots?limit=5", None),
            ("prebook", "POST", "/api/v1/micro/prebook",
             {"lot_id": "stress_lot_a", "slots": [{"slot_index": 1}], "target_time": _target()}),
            ("confirm", "POST", "/api/v1/micro/confirm", {"prebook_id": "none"}),
            ("reserve", "POST", "/api/v1/micro/reserve",
             {"lot_id": "stress_lot_a", "slot_index": 1}),
            ("release", "POST", "/api/v1/micro/release", {"slot_id": 1, "reservation_id": 0}),
            ("session_start", "POST", "/api/v1/sessions/start",
             {"lot_id": "stress_lot_a", "slot": 1}),
            ("session_end", "POST", "/api/v1/sessions/end", {"session_id": "none"}),
            ("session_active", "GET", "/api/v1/sessions/active/stress_lot_a", None),
            ("session_history", "GET", "/api/v1/sessions/history", None),
            ("pay_confirm", "POST", "/api/v1/payments/confirm", {"session_id": "none"}),
            ("pay_history", "GET", "/api/v1/payments/history", None),
            ("prediction", "POST", "/api/v1/predict/occupancy",
             {"occupied_slots": 10, "total_slots": 50, "occ_lag_15m": 5, "occ_lag_1h": 3, "net_flux": -2, "hour": 14}),
        ...
        for ep_name, method, url, body in protected_endpoints:
            ...
            check(f"{it_dev}B01 no-auth {ep_name}", r.status_code in (401, 403), r.text[:60])
        ...
        for i, bh in enumerate(bad_headers):
            ...
            check(f"{it_dev}B02 bad-token-{i}", r.status_code in (401, 403), r.text[:60])

        # B3 — Forged JWT with wrong secret
        from jose import jwt as pyjwt
        forged = pyjwt.encode({"sub": "hacker@evil.io", "role": "admin",
                                "exp": 9999999999}, "wrong-secret", algorithm="HS256")
        r = client.get("/api/v1/lots", headers={"Authorization": f"Bearer {forged}"})
        check(f"{it_dev}B03 forged jwt", r.status_code in (401, 403), r.text[:60])
        ...
        for i, sq in enumerate(sqli_payloads):
            ...
            check(f"{it_dev}B04 sqli-login-{i}", r.status_code in (401, 403, 422), r.text[:60])

        # B5 — Admin gate: non-admin cannot seed / admin ops
        h_normal = _auth(f"normal_{it}@test.io")
        admin_ops = [
            ("seed", "POST", f"/api/v1/micro/lots/stress_lot_a/slots/seed", None),
            ("create_lot", "POST", "/api/v1/lots",
             {"lot_id": f"hack_lot_{it}", "name": "x", "total_slots": 10, "base_price": 5.0}),
        ]
        for op_name, method, url, body in admin_ops:
            r = client.post(url, json=body or {}, headers=h_normal)
            check(f"{it_dev}B05 admin-gate {op_name}", r.status_code in (401, 403), r.text[:60])
        ...
        # Start session as A
        r = client.post("/api/v1/sessions/start",
                        json={"lot_id": "stress_lot_a", "slot": 50 + it},
                        headers=h_a)
        if r.status_code == 200:
            sid_a = r.json()["session_id"]
            # Try to end session A using B's token
            r = client.post("/api/v1/sessions/end", json={"session_id": sid_a}, headers=h_b)
            check(f"{it_dev}B06 IDOR end-session", r.status_code == 403, r.text[:60])
            # Try confirm payment as B
            r = client.post("/api/v1/payments/confirm", json={"session_id": sid_a}, headers=h_b)
            check(f"{it_dev}B07 IDOR pay-confirm", r.status_code == 403, r.text[:60])

            # End A's session properly
            client.post("/api/v1/sessions/end", json={"session_id": sid_a}, headers=h_a)
    ...
    if it >= 10 or it % 2 == 0:
        try:
            ...
            # C1: N concurrent prebooks on same slot (separate HTTP clients per thread)
            from fastapi.testclient import TestClient
            results = [None] * N_RACE
            clr_limits()
            with get_session() as s_check:
                slot_db = s_check.query(MicroSlot).filter(
                    MicroSlot.lot_id == "stress_lot_a",
                    MicroSlot.slot_index == race_slot,
                ).first()
                pre_state = slot_state_engine.get_state(slot_db.id) if slot_db else "NO_SLOT"
            check(f"{it_dev}C00 pre-state slot {race_slot}", pre_state == SlotState.AVAILABLE,
                  f"state={pre_state}")
            race_headers = [_auth(f"race_pb_{it}_{idx}@test.io") for idx in range(N_RACE)]
            def race_prebook(idx):
                try:
                    c = TestClient(app)
                    r = c.post("/api/v1/micro/prebook", json={
                        "lot_id": "stress_lot_a",
                        "slots": [{"slot_index": race_slot, "priority": 1}],
                        "target_time": _target(),
                    }, headers=race_headers[idx])
                    results[idx] = r.status_code
                except: results[idx] = -1
            threads = [threading.Thread(target=race_prebook, args=(i,)) for i in range(N_RACE)]
            for t in threads: t.start()
            for t in threads: t.join()
            wins = sum(1 for r in results if r == 200)
            losses = sum(1 for r in results if r in (409,))
            check(f"{it_dev}C01 race-prebook ({N_RACE} thr): {wins} win/{sum(1 for r in results if r==-1)} err",
                  wins == 1 and wins + losses == N_RACE, str(results))

            # Clean up — confirm the winner's prebook via winner's client
            if wins == 1:
                winner_idx = results.index(200)
                c_win = TestClient(app)
                pbs = c_win.get("/api/v1/sessions/history", headers=race_headers[winner_idx])
                from src.micro.state_engine import slot_state_engine
                prebooked_sid = None
                with lock:
                    for sid, drv in list(slot_state_engine._prebook_drivers.items()):
                        if drv == f"race_pb_{it}_{winner_idx}@test.io":
                            prebooked_sid = sid
                            break
                if prebooked_sid:
                    slot_state_engine.confirm_prebook(prebooked_sid, f"race_pb_{it}_{winner_idx}@test.io")

            # C2: N concurrent reserves on same slot (separate TestClient per thread)
            race_slot2 = 155 + it
            results2 = [None] * N_RACE
            clr_limits()
            race2_headers = [_auth(f"race_rs_{it}_{idx}@test.io") for idx in range(N_RACE)]
            def race_reserve(idx):
                try:
                    c = TestClient(app)
                    r = c.post("/api/v1/micro/reserve", json={
                        "lot_id": "stress_lot_a", "slot_index": race_slot2,
                    }, headers=race2_headers[idx])
                    results2[idx] = r.status_code
                except: results2[idx] = -1

            threads = [threading.Thread(target=race_reserve, args=(i,)) for i in range(N_RACE)]
            for t in threads: t.start()
            for t in threads: t.join()
            rwins = sum(1 for r in results2 if r == 200)
            rloss = sum(1 for r in results2 if r in (409,))
            check(f"{it_dev}C02 race-reserve ({N_RACE} thr): {rwins} win",
                  rwins == 1 and rwins + rloss == N_RACE, str(results2))

            # C3: prebook + reserve interleaved on same slot (must be mutual-exclusive)
            inter_slot = 160 + it
            r3a = [None]; r3b = [None]
            clr_limits()
            ...
            def _c3a():
                c = TestClient(app)
                r3a[0] = c.post("/api/v1/micro/prebook", json={
                    "lot_id": "stress_lot_a", "slots": [{"slot_index": inter_slot, "priority": 1}],
                    "target_time": _target(),
                }, headers=h3a).status_code
            def _c3b():
                c = TestClient(app)
                r3b[0] = c.post("/api/v1/micro/reserve", json={
                    "lot_id": "stress_lot_a", "slot_index": inter_slot,
                }, headers=h3b).status_code
            ...
            a_ok = r3a[0] == 200
            b_ok = r3b[0] == 200
            check(f"{it_dev}C03 prebook+reserve mutual", a_ok != b_ok,
    ...
    try:
        ...
        r = client.post("/api/v1/micro/prebook", json={
            "lot_id": "stress_lot_a", "slots": [{"slot_index": 180 + it, "priority": 1}],
            "target_time": past,
        }, headers=h_d)
        if r.status_code == 200:
            pid = r.json()["prebook_id"]
            r = client.post("/api/v1/micro/confirm", json={"prebook_id": pid}, headers=h_d)
            check(f"{it_dev}D1 expired confirm", r.status_code in (410, 409), r.text[:60])

        # D2: Nonexistent lot
        r = client.post("/api/v1/micro/prebook", json={
            "lot_id": "no_such_lot_exists_xyz", "slots": [{"slot_index": 1}],
            "target_time": _target(),
        }, headers=h_d)
        check(f"{it_dev}D2 bad lot", r.status_code == 404, r.text[:60])

        # D3: Nonexistent slot probability
        r = client.get("/api/v1/micro/lots/stress_lot_a/slots/99999/probability", headers=h_d)
        check(f"{it_dev}D3 bad slot prob", r.status_code == 404, r.text[:60])

        # D4: Double release
        r = client.post("/api/v1/micro/release", json={"reservation_id": 999999, "slot_id": 999}, headers=h_d)
        check(f"{it_dev}D4 bogus release", r.status_code in (404, 400), r.text[:60])

        # D5: Invalid target_time format
        r = client.post("/api/v1/micro/prebook", json={
            "lot_id": "stress_lot_a", "slots": [{"slot_index": 1}],
        ...
        r = client.post("/api/v1/micro/reserve", json={
            "lot_id": "stress_lot_a", "slot_index": 70 + it,
        }, headers=h_d2)
        if r.status_code == 200:
            r2 = client.get("/api/v1/micro/lots/stress_lot_a/slots?limit=100", headers=h_d)
            if r2.status_code == 200:
                sd = r2.json()
                check(f"{it_dev}D6 reserved count > 0", sd.get("reserved", 0) > 0 or sd.get("available", 0) > 0, str(sd))

        # D7: Reserve on PREBOOKED slot
        r1 = client.post("/api/v1/micro/prebook", json={
            "lot_id": "stress_lot_a", "slots": [{"slot_index": 75 + it, "priority": 1}],
            "target_time": _target(),
        }, headers=h_d)
        if r1.status_code == 200:
            h_d3 = _auth(f"edge_wrong_{it}@test.io")
            r2 = client.post("/api/v1/micro/reserve", json={
                "lot_id": "stress_lot_a", "slot_index": 75 + it,
            }, headers=h_d3)
            check(f"{it_dev}D7 reserve prebooked", r2.status_code == 409, r2.text[:60])

        # D8: Confirm prebook as wrong driver
        h_d4 = _auth(f"edge_wrong2_{it}@test.io")
        r = client.post("/api/v1/micro/prebook", json={
            "lot_id": "stress_lot_a", "slots": [{"slot_index": 80 + it, "priority": 1}],
            "target_time": _target(),
        }, headers=h_d)
        if r.status_code == 200:
            pid = r.json()["prebook_id"]
            r = client.post("/api/v1/micro/confirm", json={"prebook_id": pid}, headers=h_d4)
            check(f"{it_dev}D8 wrong driver confirm", r.status_code in (401, 403, 404), r.text[:60])

        # D9: Release without owning reservation
        r = client.post("/api/v1/micro/reserve", json={
            "lot_id": "stress_lot_a", "slot_index": 85 + it,
        }, headers=h_d)
        if r.status_code == 200:
            ...
            r2 = client.post("/api/v1/micro/release", json={
                "reservation_id": rid, "slot_id": sid,
            }, headers=h_d5)
            check(f"{it_dev}D9 release wrong owner", r2.status_code in (400, 404), r2.text[:60])

        # D10: Prebook = 0 slots (should fail schema validation)
        r = client.post("/api/v1/micro/prebook", json={
            "lot_id": "stress_lot_a", "slots": [], "target_time": _target(),
        }, headers=h_d)
        ...
        r = client.post("/api/v1/micro/prebook", json={
            "lot_id": "stress_lot_a",
            "slots": [{"slot_index": i, "priority": i} for i in range(1, 6)],
            "target_time": _target(),
        }, headers=h_d)
        ...
        r = client.post("/api/v1/micro/prebook", json={
            "lot_id": "stress_lot_a", "slots": [{"slot_index": 0}],
            "target_time": _target(),
        }, headers=h_d)
        ...
        check(f"{it_dev}D13 session no-auth", r.status_code in (401, 403), r.text[:60])

        # D14: start session on nonexistent lot
        r = client.post("/api/v1/sessions/start",
                        json={"lot_id": "no_such_lot_404", "slot": 1}, headers=h_d)
        check(f"{it_dev}D14 session bad lot", r.status_code == 404, r.text[:60])
    ...
    if it <= 20:
        try:
            ...
            r = client.post("/api/v1/lots", json={
                "lot_id": lid, "name": f"Admin Lot {it}",
                "total_slots": 50, "base_price": 15.0,
            }, headers=h_adm)
            check(f"{it_dev}E1 create lot", r.status_code in (200, 409), r.text[:60])

            # E2: Seed slots
            r = client.post(f"/api/v1/micro/lots/{lid}/slots/seed", headers=h_adm)
            check(f"{it_dev}E2 seed slots", r.status_code in (200, 500), r.text[:60])
            if r.status_code == 200:
                check(f"{it_dev}E3 seed count > 0", r.json().get("count", 0) > 0, r.text[:60])

            # E3: Update lot
            r = client.put(f"/api/v1/lots/{lid}/config", json={
                "base_price": 12.0, "price_cap": 100.0,
            }, headers=h_adm)
            check(f"{it_dev}E4 update lot", r.status_code in (200, 404), r.text[:60])
            ...
            check(f"{it_dev}E6 mine block", r.status_code in (200, 400, 429), r.text[:60])
            ...
            check(f"{it_dev}E8 revenue overview", r.status_code == 200, r.text[:60])

            # E8: Dashboard
            r = client.get("/api/v1/admin/dashboard", headers=h_adm)
            check(f"{it_dev}E9 dashboard", r.status_code in (200, 404), r.text[:60])
    ...
    try:
        ...
        if r.status_code == 200:
            sd = r.json()
            sid = sd["session_id"]
            check(f"{it_dev}F2 all layers activated",
                  all(k in sd for k in ("iot_consensus", "digital_twin", "blockchain_ref",
                                         "price_multiplier", "predicted_occupancy")), str(list(sd.keys())))
            check(f"{it_dev}F3 6-layer output",
                  len(sd.get("layers_activated", [])) >= 4, str(sd.get("layers_activated", [])))

            # End
            r = client.post("/api/v1/sessions/end", json={"session_id": sid}, headers=h_f)
            _flow_assert(f"{it_dev}F4 end session", r, 200)
            if r.status_code == 200:
                ed = r.json()
                check(f"{it_dev}F5 price same or higher",
                      ed["final_price"] >= ed["entry_price"] or abs(ed["final_price"] - ed["entry_price"]) < 0.01,
                      f"entry={ed['entry_price']} final={ed['final_price']}")

        # F2: Dual session prevention
        h_f2 = _auth(f"sess2_{it}@test.io")
        r1 = client.post("/api/v1/sessions/start",
                         json={"lot_id": "stress_lot_c", "slot": 50 + it},
                         headers=h_f2)
        r2 = client.post("/api/v1/sessions/start",
                         json={"lot_id": "stress_lot_c", "slot": 51 + it},
                         headers=h_f2)
        if r1.status_code == 200:
            check(f"{it_dev}F6 dual session blocked", r2.status_code == 409,
                  f"r1={r1.status_code} r2={r2.status_code}")

        # F3: Session history pagination
        r = client.get("/api/v1/sessions/history?offset=0&limit=10", headers=h_f)
        ...
        if r.status_code == 200:
            check(f"{it_dev}F9 fresh history empty", r.json()["total_sessions"] == 0,
    ...
    try:
        ...
        r = client.post("/api/v1/predict/occupancy", json={
            "occupied_slots": 25 + it, "total_slots": 200,
            "occ_lag_15m": 20 + it, "occ_lag_1h": 18 + it,
            "net_flux": -3 + it, "hour": it % 24,
        }, headers=h_g)
        _flow_assert(f"{it_dev}G2 predict", r, (200, 503, 429))
        if r.status_code == 200:
            pd_resp = r.json()
            check(f"{it_dev}G3 ensemble [0,1]", 0 <= pd_resp.get("ensemble_prediction", 0) <= 1,
                  str(pd_resp))
            check(f"{it_dev}G4 rf pred", "rf_prediction" in pd_resp, str(list(pd_resp.keys())))

        # G3: Pricing adjustment
        r = client.post("/api/v1/pricing/adjust", json={
        ...
        if r.status_code == 200:
            sc_list = r.json()
            check(f"{it_dev}G7 scenarios exist", isinstance(sc_list, list) and len(sc_list) > 0,
                  str(sc_list)[:80])

        # G5: Run scenario
        r = client.post("/api/v1/digital-twin/scenarios/run", json={
        ...
        r = client.post("/api/v1/digital-twin/scenario", json={
        ...
        # G7: Ingest occupancy
        r = client.post("/api/v1/ingestion/occupancy", json={
            "lot_id": "stress_lot_a", "occupied_slots": 100 + it, "total_slots": 200,
            "net_flux": 5.0,
        }, headers=_admin())
    ...
    if it % 3 == 0:
        try:
            ...
            r = client.post("/api/v1/micro/reserve", json={
                "lot_id": "stress_lot_a", "slot_index": 90 + it,
            }, headers=h_h)
            if r.status_code == 200:
                e_state = slot_state_engine.get_state(r.json()["slot_id"])
                check(f"{it_dev}H1 engine matches RESERVED", e_state == SlotState.RESERVED,
                      f"got {e_state}")

                # release
                rid = r.json()["reservation_id"]
                sid = r.json()["slot_id"]
                client.post("/api/v1/micro/release", json={
                    "reservation_id": rid, "slot_id": sid,
                }, headers=h_h)
                e_state2 = slot_state_engine.get_state(sid)
                check(f"{it_dev}H2 engine AVAILABLE after release", e_state2 == SlotState.AVAILABLE,
                      f"got {e_state2}")
            ...
            if r.status_code == 200:
                ...
                db2 = get_session()
                try:
                    ms = db2.query(MicroSlot).filter(
                        MicroSlot.lot_id == "stress_lot_a",
                        MicroSlot.slot_index == slot_idx,
                    ).first()
                    if ms:
                        e_st = slot_state_engine.get_state(ms.id)
                        check(f"{it_dev}H3 engine PREBOOKED match", e_st == SlotState.PREBOOKED,
                              f"got {e_st}")
                finally:
                    db2.close()
            ...
            with get_session() as db3:
                all_slots = db3.query(MicroSlot).filter(
                    MicroSlot.lot_id == "stress_lot_a", MicroSlot.active == 1,
                ).limit(50).all()
            occ = se.occupancies("stress_lot_a", all_slots)
            for key in ("total_slots", "available_slots", "reserved_slots",
                         "occupied_slots", "prebooked_slots", "occupancy_rate"):
                check(f"{it_dev}H5 occ has {key}", key in occ, str(list(occ.keys())))
    ...
    if it >= 25:
        try:
            ...
            for attempt in range(10):
                r = client.post("/api/v1/micro/prebook", json={
                    "lot_id": "stress_lot_a",
                    "slots": [{"slot_index": 190 + attempt, "priority": 1}],
                    "target_time": _target(),
                }, headers=h_i)
                if r.status_code == 429:
                    check(f"{it_dev}I1 rate limit hit at attempt {attempt+1}", True, "")
                    break
            else:
                check(f"{it_dev}I1 rate limit NOT hit (may be config)", False,
            ...
            for attempt in range(10):
                rl = client.post("/api/v1/auth/login",
                                 json={"email": f"ratelimit_{it}@test.io", "password": "WrongPass!"})
                last_status = rl.status_code
                if rl.status_code == 429:
                    hit_login = True
                    break
            check(f"{it_dev}I2 login rate limit", True,
