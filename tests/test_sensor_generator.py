...
def test_simulator_initialization():
    sim = RealisticParkingSensorSimulator("test_zone", capacity=20, base_price=12.5)
...
def test_base_occupancy_rates():
    ...
    # (Since there is stochastic noise in the simulator, we check statistical bounds or expected behavior)
...
def test_spatial_skew():
    sim = RealisticParkingSensorSimulator("test_zone", capacity=100, entrance_skew=25.0)
    ...
    # Spots closer to the entrance (indices 0-20) should have a significantly higher
    # fill rate than spots far from the entrance (indices 80-100)
...
def test_sample_step_output():
    ...
    for r in readings:
        ...
        assert r.is_false_positive == (r.ultrasonic_occupied != r.vision_occupied)
