...
def test_landing_page_loads(page):
    ...
    assert "SMART PARKING ECOSYSTEM" in body_upper, (
        f"Portal selector missing subtitle: {body[:200]}"
    )
    assert "Driver Portal" in body, f"Driver portal missing: {body[:200]}"
    assert "Operator & Admin Portal" in body, (
        f"Admin portal missing: {body[:200]}"
    )
    assert "AI · MARL · BLOCKCHAIN · CITY-SCALE" in body_upper, (
        f"Architecture tagline missing: {body[:200]}"
    )
