...
def _pre_register_users():
    _ensure_user(
        "e2e@test.io", "E2ePass123!", "E2E User", retries=10, role="admin"
    )
    _ensure_user(
        "nav@test.io", "NavPass123!", "Nav User", retries=10, role="admin"
    )
    ...
    for attempt in range(retries + 1):
        ...
        try:
            ...
            try:
                ...
                pass  # nosec — best-effort read error body
            if e.code == 429 and attempt < retries:
                continue
            raise
...
def page(context):
    ...
    p.on(
        "requestfailed",
        lambda req: print(
            f"[REQ FAILED] {req.method} {req.url}: "
            f"{getattr(req.failure, 'error_text', req.failure) or 'no error'}"
        ),
    )
    p.on(
        "response",
...
def _wait_for_spa(page, timeout=15):
    ...
    print(
        ...
        f"[DEBUG _wait_for_spa] Text:\n{text}\n"
    )
    raise AssertionError(
        f"SPA admin UI never appeared after {timeout}s. URL: {url}"
    )
...
def login(page, email="brenda@pragma.io", password="TestPass123!"):
    ...
    # Set HttpOnly cookie before SPA mounts. AuthProvider only calls
    # /auth/me on initial mount; navigating first would cache an
    # unauthenticated state and redirect to /login even after the
    # cookie is added.
