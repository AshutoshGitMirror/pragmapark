...
class TestIDOR:
    def test_active_sessions_scoped_by_driver(self, client, auth_headers):
        resp = client.get(
            "/api/v1/sessions/active/test_lot", headers=auth_headers
        )
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_end_session_idor(self, client, auth_headers):
        resp = client.post(
            "/api/v1/sessions/end/nonexistent", headers=auth_headers
        )
...
class TestRoleIsolation:
    def test_driver_cannot_see_other_driver_detail(self, client):
        token_a = _register_or_login(
            client, "driver_a_147@pragma.io", "TestPass123!", "Driver A"
        )
        headers_a = {"Authorization": f"Bearer {token_a}"}
        token_b = _register_or_login(
            client, "driver_b_147@pragma.io", "TestPass123!", "Driver B"
        )
    ...
    def test_driver_cannot_end_other_driver_session(self, client):
        token_a = _register_or_login(
            client, "driver_c_147@pragma.io", "TestPass123!", "Driver C"
        )
        headers_a = {"Authorization": f"Bearer {token_a}"}
        token_b = _register_or_login(
            client, "driver_d_147@pragma.io", "TestPass123!", "Driver D"
        )
        headers_b = {"Authorization": f"Bearer {token_b}"}
        ...
        client.cookies.clear()
        resp = client.post(
            "/api/v1/sessions/end", json={"session_id": sid}, headers=headers_b
        )
        assert resp.status_code == 403

    def test_driver_cannot_list_other_driver_active(self, client):
        token_a = _register_or_login(
            client, "driver_e_147@pragma.io", "TestPass123!", "Driver E"
        )
        headers_a = {"Authorization": f"Bearer {token_a}"}
        token_b = _register_or_login(
            client, "driver_f_147@pragma.io", "TestPass123!", "Driver F"
        )
        headers_b = {"Authorization": f"Bearer {token_b}"}
        ...
        client.post(
            "/api/v1/sessions/start",
            json={"lot_id": "iso_lot3", "slot": 3},
            headers=headers_a,
        )

        client.cookies.clear()
        resp = client.get(
            "/api/v1/sessions/active/iso_lot3", headers=headers_b
        )
        assert resp.status_code == 200
...
class TestDedupStart:
    def test_duplicate_start_same_lot_slot_returns_409(
        self, client, auth_headers
    ):
        _create_lot("dedup_lot")
        r1 = client.post(
            "/api/v1/sessions/start",
            json={"lot_id": "dedup_lot", "slot": 10},
            headers=auth_headers,
        )
    ...
    def test_duplicate_start_different_lot_ok(self, client, auth_headers):
        ...
        r1 = client.post(
            "/api/v1/sessions/start",
            json={"lot_id": "dedup_lot_a", "slot": 1},
            headers=auth_headers,
        )
        assert r1.status_code == 200
        ...
        r2 = client.post(
            "/api/v1/sessions/start",
            json={"lot_id": "dedup_lot_b", "slot": 1},
            headers=auth_headers,
        )
