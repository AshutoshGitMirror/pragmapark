...
def _seed_db():
    ...
    _rm()
    assert "token_blacklist" in sa.inspect(engine).get_table_names(), (
    ...
    try:
        if (
            not db.query(User)
            .filter(User.email == "admin@usertest.io")
            .first()
        ):
            db.add(
                User(
                    email="admin@usertest.io",
...
class TestFrustratedFrank:
    ...
    def test_frank_mashes_prebook(self, client):
        """Frank prebooks same slot 5x rapid-fire → only 1st succeeds."""
        ...
        assert all(s == 409 for s in results[1:]), (
            f"Follow-ups should all 409: {results}"
        )

    def test_frank_double_confirm(self, client):
        """Frank mashes Confirm → second returns error, no double-charge."""
    ...
    def test_frank_reserves_already_occupied_slot(self, client):
        """Frank tries to reserve an already-reserved slot → 409."""
...
class TestNetworkNancy:
    ...
    def test_nancy_double_payment_confirm(self, client):
        ...
        p1 = client.post(
            "/api/v1/payments/confirm", json={"session_id": sid}, headers=h
        )
        p2 = client.post(
            "/api/v1/payments/confirm", json={"session_id": sid}, headers=h
        )
        if ok(p1, 200):
            ref1 = p1.json().get("data", {}).get(
                "blockchain_ref"
            ) or p1.json().get("blockchain_ref", "")
            assert ref1 or True
            if ok(p2, 200):
                ref2 = p2.json().get("data", {}).get(
                    "blockchain_ref"
                ) or p2.json().get("blockchain_ref", "")
                assert ref1 == ref2, f"bc_ref mismatch: {ref1} vs {ref2}"

    def test_nancy_session_end_twice(self, client):
        """Nancy hits End → network blips → hits End again → 404 on 2nd."""
    ...
    def test_nancy_prebook_duplicate_payload(self, client):
        """Nancy submits same prebook twice → idempotent (once only)."""
        h = _auth(client, f"nancy3_{uuid.uuid4().hex[:6]}@test.io")
        clr()
        payload = {
            "lot_id": "ut_lot_a",
            "slots": [{"slot_index": 12, "priority": 1}],
            "target_time": _target(),
        }
        ...
        assert r2.status_code != 200, (
            f"2nd prebook should not double-book: {r2.text}"
        )
...
class TestCheatingCharlie:
    ...
    def test_charlie_confirm_as_wrong_user(self, client):
        ...
        assert c.status_code in (403, 404, 409), (
            f"wrong-user confirm: {c.text}"
        )
    ...
    def test_charlie_negative_price_payment(self, client):
        ...
        assert r.status_code == 200, (
            f"neg amount ignored → normal payment: {r.text}"
        )
        js = r.json()
        assert js.get("amount", -50) != -50, (
            f"should use DB amount, not user-supplied: {js}"
        )
    ...
    def test_charlie_idor_other_session(self, client):
        ...
        sid = r.json()["session_id"]
        r = client.post(
            "/api/v1/sessions/end", json={"session_id": sid}, headers=h_b
        )
...
class TestGPSGloria:
    ...
    def test_gloria_sees_slot_state(self, client):
        ...
        clr()
        r = client.get(
            "/api/v1/micro/lots/ut_lot_a/slots/1/probability", headers=h
        )
...
class TestAdminAndy:
    ...
    def test_andy_creates_lot_under_load(self, client):
        ...
        clr()
        threads = [
            th.Thread(target=prebook_worker, args=(i,)) for i in range(5)
        ]
        for t in threads:
            t.start()
        r = client.post(
    ...
    def test_andy_revenue_report(self, client):
        """Andy runs revenue report — gets data or perm error if not admin."""
        h_adm = _admin(client)
        clr()
    ...
    def test_zoe_confirm_expired_prebook(self, client):
        ...
        if ok(r, 200):
            ...
            assert c.status_code != 200, (
                f"expired prebook should not confirm: {c.text}"
            )
