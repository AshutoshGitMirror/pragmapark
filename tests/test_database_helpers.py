...
from src.api.database import (
    get_session, get_db, get_latest_occupancies, get_recent_records,
    lot_to_summary, OccupancyRecord, ParkingLot, Base, get_engine,
)
...
class TestGetLatestOccupancies:
    def test_returns_empty_for_empty_list(self):
        assert get_latest_occupancies(get_session(), []) == {}
    ...
    def test_returns_latest_record_per_lot(self):
        ...
        db.commit()
        result = get_latest_occupancies(get_session(), ["test_lot"])
...
class TestGetRecentRecords:
    def test_returns_empty_for_unknown_lot(self):
        assert get_recent_records(get_session(), "nonexistent") == []
    ...
    def test_returns_limited_records(self):
        ...
        db.commit()
        result = get_recent_records(get_session(), "test_lot", limit=5)
        assert len(result) == 5
