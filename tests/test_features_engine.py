...
import pandas as pd
import numpy as np
from src.features.engine import process_raw_to_features, build_features_from_records
...
class TestProcessRawToFeatures:
    ...
    def test_feature_columns_present(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = process_raw_to_features(path)
            for col in ["hour_sin", "hour_cos", "occ_lag_15m", "pe_net_flux", "is_weekend"]:
...
class TestBuildFeaturesFromRecords:
    ...
    def test_none_when_one(self):
        records = [{"occupancy_rate": 0.5, "timestamp": pd.Timestamp("2025-01-01 12:00")}]
        assert build_features_from_records(records, 100) is None

    def test_returns_series_with_two(self):
        records = [
            {"occupancy_rate": 0.4, "occupied_slots": 40, "timestamp": pd.Timestamp("2025-01-01 12:00")},
            {"occupancy_rate": 0.5, "occupied_slots": 50, "timestamp": pd.Timestamp("2025-01-01 12:15")},
        ]
    ...
    def test_handles_records_without_occupancy_rate(self):
        records = [
            {"occupied_slots": 40, "timestamp": pd.Timestamp("2025-01-01 12:00")},
            {"occupied_slots": 50, "timestamp": pd.Timestamp("2025-01-01 12:15")},
        ]
        result = build_features_from_records(records, 100)
        assert result is not None
