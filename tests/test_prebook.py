...
def _clear_prebook_limiter():
    ...
    with Session(bind=get_engine()) as s:
        s.query(RateLimitWindow).filter(
            RateLimitWindow.key.like("prebook:%")
        ).delete()
        s.commit()
...
class TestPrebookAPI:
    ...
    def seeded_lot(self, client, admin_headers):
        ...
        client.post(
            "/api/v1/micro/lots/prebook_test_lot/slots/seed",
            headers=admin_headers,
        )
    ...
    def test_prebook_no_slots_available(
        ...
        # Top up alt driver so wallet check passes before slot check
        client.post(
            "/api/v1/wallet/topup",
            json={"amount": 100.0},
    ...
    def test_prebook_no_show_detection(
        self, client, auth_headers, admin_headers, seeded_lot
    ):
        past = (datetime.now(timezone.utc) - timedelta(hours=2)).isoformat()
        prebook = client.post(
