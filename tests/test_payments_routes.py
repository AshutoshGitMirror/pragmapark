...
def _create_lot_and_session(client, auth_headers, lot_id="pay_lot"):
    ...
    try:
        if not db.query(ParkingLot).filter(ParkingLot.lot_id == lot_id).first():
            db.add(ParkingLot(lot_id=lot_id, name="Pay Test", total_slots=100, base_price=10.0, price_cap=50.0))
            db.commit()
    finally:
        db.close()
    start = client.post("/api/v1/sessions/start", json={"lot_id": lot_id, "slot": 1}, headers=auth_headers)
...
class TestConfirmPayment:
    def test_confirm_requires_auth(self, client):
        resp = client.post("/api/v1/payments/confirm", json={"session_id": "x"})
        assert resp.status_code in (401, 403)

    def test_confirm_returns_404_bad_session(self, client, auth_headers):
        resp = client.post("/api/v1/payments/confirm", json={"session_id": "bad"}, headers=auth_headers)
        assert resp.status_code == 404

    def test_confirm_success(self, client, auth_headers):
        sid = _create_lot_and_session(client, auth_headers)
        resp = client.post("/api/v1/payments/confirm", json={"session_id": sid}, headers=auth_headers)
    ...
    def test_confirm_returns_already_paid(self, client, auth_headers):
        sid = _create_lot_and_session(client, auth_headers)
        client.post("/api/v1/payments/confirm", json={"session_id": sid}, headers=auth_headers)
        resp = client.post("/api/v1/payments/confirm", json={"session_id": sid}, headers=auth_headers)
        assert resp.status_code == 200
