...
class TestAdjustPrice:
    ...
    def test_adjust_requires_auth(self, client):
        resp = client.post(
            ...
            json={"predicted_occupancy": 0.5, "current_price": 10.0},
        )
...
class TestLotPricing:
    def test_lot_pricing_public(self, client):
        ...
        # Returns demo pricing when DB empty (public, no auth)
        assert resp.status_code == 200

    def test_lot_pricing_returns_404_for_unknown(self, client, auth_headers):
        resp = client.get(
            "/api/v1/pricing/lots?lot_id=nonexistent", headers=auth_headers
        )
        # Demo pricing when DB empty; 404 only if lots exist but no match
        assert resp.status_code in (200, 404)

    def test_lot_pricing_returns_lot_data(self, client, auth_headers):
        _create_lot_with_occ()
        resp = client.get(
            "/api/v1/pricing/lots?lot_id=pricing_zone", headers=auth_headers
        )
        assert resp.status_code == 200
        data = resp.json()
