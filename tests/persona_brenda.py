...
import os, sys, uuid, json, random, math
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))
os.environ["DATABASE_URL"] = f"sqlite:////tmp/persona_brenda.db"
...
from src.api.database import get_engine, get_session, Base, User, ParkingLot, MicroSlot
...
try:
    if not db.query(User).filter(User.email == "admin@personas.io").first():
        db.add(User(email="admin@personas.io", hashed_password=hash_password("Admin123!"),
        ...
        if db.query(MicroSlot).filter(MicroSlot.lot_id == lot_id).count() == 0:
            ...
            for r in range(rows):
                for p in range(per_row):
                    ...
                    st = "handicap" if random.random() < 0.05 else ("ev" if random.random() < 0.10 else "regular")
...
r = client.post("/api/v1/auth/register", json={
    "email": EMAIL, "password": PW, "full_name": "Bougie Brenda",
})
...
for lot in lots:
    print(f"   {lot['name']} — ${lot['dynamic_price']:.2f}, {lot['available_spots']} spots, NO VALET?!")
...
d = r.json()
print(f"🔍 BRENDA INSPECTS LOT A: ${d['current_price']:.2f}/hr, {d['available_spots']} avail")
...
if r.status_code == 200:
    ...
    print(f"✅ BRENDA ENDED SESSION. charged ${r.json()['amount_charged']:.2f}")
    # 7. CHECK HISTORY
    ...
    print(f"   BRENDA: 'THIS APP IS SO CHEAP.' status={r.status_code} {r.text}")

# 8. CHECK PAYMENT
r = client.get("/api/v1/sessions/history", headers=h)
print(f"💰 BRENDA: 'wait, that's it? no hidden fees?'")
print(f"   response: {r.status_code}")
js = r.json()
if js["total_sessions"] > 0:
    sess = js["sessions"][0]
    print(f"   charged: ${sess['amount_charged']:.2f}, status={sess['status']}")
