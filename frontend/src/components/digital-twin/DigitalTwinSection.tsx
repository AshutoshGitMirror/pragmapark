...
export function DigitalTwinSection() {
  ...
  return (
    <section className="section bg-deeper" id="digital-twin">
      <div className="section-inner">
        ...
        {!loading && !fetchError && scenarios.length === 0 && (
          <div className="max-w-lg mx-auto p-4 rounded-xl text-xs font-mono text-center text-dim"
            style={{
          ...
          >
            {scenarios.map((s, i) => {
              ...
              return (
                <div
                  key={s.name}
                  className="min-w-[280px] bg-surface border border-[rgba(255,255,255,0.06)] rounded-xl p-6 shrink-0"
                >
                  <div className="text-2xl mb-3">{s.icon}</div>
                  <h3 className="text-base font-medium text-white mb-1">{s.name}</h3>
                  <p className="text-xs text-muted mb-4">{s.description}</p>

                  <div className="flex gap-4 mb-4 text-xs font-mono">
                    <div>
                      <span className="text-dim">Occ: </span>
                  ...
                  <motion.button
                    ...
                    className="w-full py-2 px-4 rounded-lg text-xs font-mono font-medium border border-[rgba(255,255,255,0.1)] text-muted hover:border-[#00c785] hover:text-emerald transition-all disabled:opacity-40"
                  >
                    {runningIdx === i ? (
                  ...
                  {result && result.comparisons && result.comparisons.length > 0 && (
                    <div className="mt-4 pt-4 border-t border-[rgba(255,255,255,0.04)] text-[10px] font-mono space-y-1">
                      <div className="flex justify-between">
                        ...
                        <span className="text-muted">{result.comparisons[0].price_delta}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-dim">Occupancy Change</span>
                        <span className="text-muted">{result.comparisons[0].occupancy_delta}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-dim">Congestion</span>
                        <span className="text-muted">{result.comparisons[0].congestion}</span>
                      </div>
                    </div>
                  )}
                </div>
